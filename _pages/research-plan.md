...
{% for post in selected_pubs reversed %}<div class="rp-pub"><p class="rp-pub-title"><a href="{{ post.url }}">{{ post.title }}</a></p><em class="rp-pub-venue">{{ post.venue }}</em><div class="rp-pub-content">{% include archive-single.html %}</div></div>{% endfor %}
...
<script>
document.querySelectorAll('.rp-pub-venue').forEach(function(el) {
  el.addEventListener('click', function() {
    el.closest('.rp-pub').classList.toggle('open');
  });
});
</script>
