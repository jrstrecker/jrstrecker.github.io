...
{% for post in selected_pubs reversed %}<details class="pub-collapsible"><summary><span>{{ post.title }}<em class="pub-collapsible-venue">{{ post.venue }}</em></span></summary>{% include archive-single.html %}</details>{% endfor %}
